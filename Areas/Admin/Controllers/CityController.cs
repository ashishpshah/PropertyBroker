    ...
    public class CityController : BaseController<ResponseModel<City>>
    {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        CommonViewModel.ObjList.Add(new City()
                        {
                            ...
                            CityName = dr["Name"] != DBNull.Value ? Convert.ToString(dr["Name"]) : "",
                ...
                if (Id > 0)
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        obj = new City()
                        {
                            ...
                            CityName = dt.Rows[0]["Name"] != DBNull.Value ? Convert.ToString(dt.Rows[0]["Name"]) : "",
        ...
        public JsonResult Save(City viewModel)
        {
            try
            {
                ...
                oParams.Add(new SqlParameter("@CityName", SqlDbType.VarChar) { Value = viewModel.CityName ?? "" });
