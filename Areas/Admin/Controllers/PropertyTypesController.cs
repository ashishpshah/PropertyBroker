...
namespace Broker.Areas.Admin.Controllers
{
	...
	public class PropertyTypesController : BaseController<ResponseModel<PropertyType>>
	{
		...
		public ActionResult IndexSubType(long Parent_Id = 0)
		{
			...
			if (dt != null && dt.Rows.Count > 0) CommonViewModel.Obj.Name = dt.Rows[0]["Name"].ToString();
		...
		public ActionResult Partial_AddEditForm(long Id = 0)
		{
			...
			if (Id > 0) CommonViewModel.Obj = DataContext_Command.PropertyType_Get(Id).FirstOrDefault();

			CommonViewModel.SelectListItems = new List<SelectListItem_Custom>();

			var list = _context.Using<PropertyCategory>().GetByCondition(x => x.IsActive == true).OrderBy(x => x.Name)
		...
		public ActionResult Save(PropertyType viewModel)
		{
			try
			{
				if (viewModel != null && viewModel != null)
				{
					...
					if (string.IsNullOrEmpty(viewModel.SelectedCategoryId_Str))
					{
						CommonViewModel.IsSuccess = false;
						CommonViewModel.StatusCode = ResponseStatusCode.Error;
